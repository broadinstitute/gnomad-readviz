...
N_SAMPLES_PER_CLUSTER=3432  # since each sample id takes ~15 seconds, each cluster should take cluster ~14.5 hours, and 20 clusters are needed to process all 68339 sample ids
...
do
	...
	hailctl dataproc start --num-workers ${N_WORKERS} --num-preemptible ${N_PREEMPTIBLES} --max-idle 10m --project ${PROJECT} --init ${INIT_ARG} --packages ${PACKAGES_ARG} ${CLUSTER_NAME} \
    	...
    	&& hailctl dataproc submit ${CLUSTER_NAME} export_tsvs.py --start-with-sample-i ${i} --n-samples-to-process ${N_SAMPLES_PER_CLUSTER} ${SAMPLE_IDS_PATH} \
