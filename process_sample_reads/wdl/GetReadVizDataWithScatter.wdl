...
task RunHaplotypeCallerBamout {
	...
	runtime {
		docker: "weisburd/gnomad-readviz@sha256:283118579cbe8e2d4ec6b4fb3bc83a6950ac9a05fa7bebec9b45540859961a8d"
		cpu: 1
		preemptible: 0
		memory: "8 GiB"
...
workflow GetReadVizDataWorkflow {
	input {
		Array[File] input_cram_array
		Array[File] input_crai_array
		Array[File] variants_tsv_bgz_array
    ...
    scatter(input_files in zip(zip(input_cram_array, input_crai_array), variants_tsv_bgz_array)) {
        call RunHaplotypeCallerBamout {
            input:
                variants_tsv_bgz = input_files.right,
                input_cram = input_files.left.left,
                input_crai = input_files.left.right,
                ref_fasta = ref_fasta,
                ref_fasta_fai = ref_fasta_fai,
                ref_fasta_dict = ref_fasta_dict,
                requester_pays_project = requester_pays_project,
                output_prefix = sub(basename(input_files.left.left), "\\.cram$", ""),
	...
	output {
		...
		Array[File] output_bamout_cram = RunHaplotypeCallerBamout.output_bamout_bam
		Array[File] output_bamout_cram_crai = RunHaplotypeCallerBamout.output_bamout_bai
		Array[File] output_gvcf_gz = RunHaplotypeCallerBamout.output_gvcf_gz
		Array[File] output_gvcf_gz_tbi = RunHaplotypeCallerBamout.output_gvcf_gz_tbi

		# save small intermediate files for debugging
		Array[File] input_cram_header = RunHaplotypeCallerBamout.input_cram_header
		Array[File] variant_windows_bed = RunHaplotypeCallerBamout.variant_windows_bed
		Array[File] variant_windows_interval_list = RunHaplotypeCallerBamout.variant_windows_interval_list
	}
}
