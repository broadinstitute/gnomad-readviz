...
def parse_args():
    ...
    p.add_argument("--tsv-has-header", action="store_true", help="The input tsv file has a header line")
...
def parse_tsv_with_header(tsv_path, progress_bar=False):
    """Parse tsv file, yield: (chrom, pos, ref, alt, zygosity, tsv_path, qual)"""
    ...
    with gzip.open(tsv_path, "rt") as f:
        ...
        for line in (tqdm(f, unit=f" {tsv_path} lines") if progress_bar else f):
            ...
            chrom, pos, ref, alt, zygosity_number, qual = r["chrom"], r["pos"], r["ref"], r["alt"], r["het_or_hom_or_hemi"], r["GQ"]
            pos = int(pos)
            zygosity_number = int(zygosity_number)
            qual = int(qual)
