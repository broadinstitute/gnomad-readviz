...
#SAMPLE_IDS_PATH="gs://gnomad-bw2/sample_ids_gnomad_v3__20210131.txt"
SAMPLE_IDS_PATH="gs://gnomad-bw2/sample_ids_gnomad_v3_1__20210131.txt"
...
#INPUT_HT="gs://gnomad-bw2/gnomad_v3_readviz_crams__that_failed_AB_filter_exploded_keyed_by_sample.ht"
INPUT_HT="gs://gnomad-bw2/gnomad_v3_1_readviz_crams__that_failed_AB_filter_exploded_keyed_by_sample.ht"

#OUTPUT_DIR="gs://gnomad-bw2/gnomad_v3__readviz_tsvs__variants_that_failed_AB_filter"
OUTPUT_DIR="gs://gnomad-bw2/gnomad_v3_1_readviz_tsvs__variants_that_failed_AB_filter"

#LOG_FILE_SUFFIX="gnomad-v3"
LOG_FILE_SUFFIX="gnomad-v3-1"
...
N_SAMPLES_PER_CLUSTER=900  # there are 4445 samples in gnomAD v3.1 and each sample id requires ~15 seconds to generate the tsv, so this should take ~ 4 hours using 5 clusters
...
do
    ...
    CLUSTER_NAME="${LOG_FILE_SUFFIX}-readviz-tsv-cluster${k}"
	...
	hailctl dataproc start --num-workers ${N_WORKERS} --num-preemptible ${N_PREEMPTIBLES} --max-idle 10m --project ${PROJECT} --init ${INIT_ARG} --packages ${PACKAGES_ARG} ${CLUSTER_NAME} \
	    --master-machine-type ${MASTER_MACHINE_TYPE} --worker-machine-type ${WORKER_MACHINE_TYPE} \
    	    && hailctl dataproc submit ${CLUSTER_NAME} step3__export_per_sample_tsvs.py --start-with-sample-i ${i} --n-samples-to-process ${N_SAMPLES_PER_CLUSTER} -i ${INPUT_HT} ${SAMPLE_IDS_PATH} --output-bucket-path  $OUTPUT_DIR \
    	    && gcloud --project ${PROJECT} dataproc clusters delete --quiet ${CLUSTER_NAME} \
    ) >& ${CLUSTER_NAME}.${LOG_FILE_SUFFIX}.log &
    set +x
done
